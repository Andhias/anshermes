...
function createWindow(): void {
  ...
  mainWindow.webContents.on("console-message", (details) => {
    if (details.level === "warning" || details.level === "error") {
      console.error(
        `[RENDERER ${details.level.toUpperCase()}] ${details.message} (${details.sourceId}:${details.lineNumber})`,
      );
    }
  });
...
function setupIPC(): void {
  ...
  ipcMain.handle("open-hermes-env", async (_event, profile?: string) => {
    const conn = getConnectionConfig();
    if (conn.mode === "ssh") {
      return {
        success: false,
        error: "Cannot open remote .env file from local desktop app",
        path: "~/.hermes/.env",
      };
    }
