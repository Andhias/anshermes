...
import { showToast, toastAction } from "../../components/toast";
...
function Schedules({ profile }: SchedulesProps): React.JSX.Element {
  ...
  async function handleCreate(): Promise<void> {
    ...
    try {
      ...
      if (result.success) {
        const taskLabel = newName.trim() || buildSchedule();
        closeCreateModal();
        await loadJobs();
        toastAction(t("schedules.create"), taskLabel);
      } else {
        const msg = result.error || "Failed to create job";
        setError(msg);
        showToast(msg, "error");
      }
    } catch {
  ...
  async function handleRemove(jobId: string): Promise<void> {
    ...
    try {
      ...
      if (result.success) {
        await loadJobs();
        const taskLabel = jobs.find((j) => j.id === jobId)?.name || jobId;
        toastAction(t("schedules.delete"), taskLabel);
      } else {
        const msg = result.error || "Failed to remove job";
        setError(msg);
        showToast(msg, "error");
      }
    } catch {
      const msg = "Failed to remove job";
      setError(msg);
      showToast(msg, "error");
    } finally {
      setActionInProgress(null);
  ...
  async function handleToggle(job: CronJob): Promise<void> {
    ...
    try {
      ...
      if (result.success) {
        await loadJobs();
        const taskLabel = job.name || job.id;
        showToast(
          `${job.state === "paused" ? t("schedules.resume") : t("schedules.pause")} • ${taskLabel}`,
          "success",
        );
      } else {
        const msg = result.error || "Failed to update job";
        setError(msg);
        showToast(msg, "error");
      }
    } catch {
  ...
  async function handleTrigger(jobId: string): Promise<void> {
    ...
    try {
      ...
      if (result.success) {
        await loadJobs();
        const taskLabel = jobs.find((j) => j.id === jobId)?.name || jobId;
        toastAction(t("schedules.runNow"), taskLabel);
      } else {
        const msg = result.error || "Failed to trigger job";
        setError(msg);
        showToast(msg, "error");
      }
    } catch {
      const msg = "Failed to trigger job";
      setError(msg);
      showToast(msg, "error");
    } finally {
      setActionInProgress(null);
