...
function Settings({ profile }: { profile?: string }): React.JSX.Element {
  ...
  async function handleMigrate(): Promise<void> {
    ...
    try {
      ...
      cleanup();
      if (result.success) {
        const msg = t("settings.migrationComplete");
        setMigrationResult(msg);
        setMigrationResultType("success");
        showToast(msg, "success");
        setOpenclawFound(false);
      } else {
        const msg = result.error || t("settings.migrationFailed");
        setMigrationResult(msg);
      ...
      cleanup();
      const msg = (err as Error).message || t("settings.migrationFailed");
      setMigrationResult(msg);
      setMigrationResultType("error");
      showToast(msg, "error");
    }
  ...
  async function handleTestConnection(): Promise<void> {
    if (connMode === "ssh") {
      ...
      const msg = ok ? t("settings.sshConnected") : t("settings.sshFailed");
      setConnStatus(msg);
  ...
  async function handleSwitchToLocal(): Promise<void> {
    ...
    showToast(t("settings.switchedToLocal"), "success");
    setTimeout(() => setConnStatus(null), 2000);
  }
  ...
  async function handleBackup(): Promise<void> {
    ...
    setBackingUp(false);
    if (result.success) {
      const msg = `Backup created: ${result.path || "success"}`;
      setBackupResult(msg);
      showToast(t("settings.backupCreatedToast", { path: result.path || "success" }), "success");
    } else {
      const msg = result.error || "Backup failed.";
      setBackupResult(msg);
      showToast(t("settings.backupFailedToast"), "error");
    }
  }
  ...
  async function handleUpdateHermes(): Promise<void> {
    ...
    setUpdating(false);
    if (result.success) {
      const msg = t("settings.updateSuccess");
      setUpdateResult(msg);
      setUpdateResultType("success");
      showToast(msg, "success");
      refreshVersion();
    } else {
      const msg = result.error || t("settings.updateFailed");
      setUpdateResult(msg);
