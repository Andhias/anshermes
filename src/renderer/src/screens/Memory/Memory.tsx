...
function Memory({ profile }: { profile?: string }): React.JSX.Element {
  ...
  async function handleAddEntry(): Promise<void> {
    ...
    if (result.success) {
      ...
      setShowAdd(false);
      await loadData();
      toastAction(t("memory.addMemory"));
    } else {
      const msg = result.error || t("memory.addFailed");
      setError(msg);
      showToast(msg, "error");
    }
  }
  ...
  async function handleSaveEdit(): Promise<void> {
    ...
    if (result.success) {
      ...
      setEditContent("");
      await loadData();
      toastAction(t("memory.save"));
    } else {
      const msg = result.error || t("memory.updateFailed");
      setError(msg);
      showToast(msg, "error");
    }
  }
  ...
  async function handleSaveUserProfile(): Promise<void> {
    ...
    if (result.success) {
      ...
      await loadData();
      toastAction(t("memory.saveProfile"));
    } else {
      const msg = result.error || t("memory.saveFailed");
      setError(msg);
      showToast(msg, "error");
    }
  }
