...
function App(): React.JSX.Element {
  ...
  useEffect(() => {
    const handler = (event: Event): void => {
      const custom = event as CustomEvent<{ message: string; level?: ToastLevel }>;
    ...
    window.addEventListener("hermes-toast", handler as EventListener);
    return () => window.removeEventListener("hermes-toast", handler as EventListener);
  }, []);
          ...
          {toasts.length > 0 && (
            <div className="global-toast-stack" role="status" aria-live="polite">
              {toasts.map((toast) => (
                <div
                  key={toast.id}
                  className={`global-toast global-toast-${toast.level}`}
                >
                  {toast.message}
                </div>
              ))}
